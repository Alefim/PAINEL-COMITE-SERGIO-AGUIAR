...
export default function Dashboard() {
  ...
  function emitirRelatorio() {
    ...
    const linhasRankingPorVisita = rankingPorVisita.length
      ? rankingPorVisita.map((x, i) => {
          const totalPorBairro = Object.fromEntries(opcoesCandidato.map(nome => [
            nome,
            visitasComparacao.reduce((soma, v) => soma + Number(x.visitas[v][nome] || 0), 0),
          ])) as Record<string, number>;

          const linhasVisitas = visitasComparacao.map((v, indiceVisita) => `<tr class="${indiceVisita === 0 ? "bairro-inicio" : ""}">${indiceVisita === 0 ? `<td rowspan="3">${i + 1}</td><td rowspan="3"><strong>${safe(x.bairro)}</strong></td>` : ""}<td class="visita-col">${safe(v)}</td>${opcoesCandidato.map(nome => `<td class="n">${Number(x.visitas[v][nome] || 0).toLocaleString("pt-BR")}</td>`).join("")}</tr>`).join("");

          const linhaTotal = `<tr class="bairro-total"><td></td><td></td><td class="visita-col total-col">TOTAL</td>${opcoesCandidato.map(nome => `<td class="n total-col">${Number(totalPorBairro[nome] || 0).toLocaleString("pt-BR")}</td>`).join("")}</tr>`;

          return `${linhasVisitas}${linhaTotal}`;
        }).join("")
    ...
    const tabelaPretensao = visita === "Todas as visitas"
      ? `<p>Comparativo da 1ª visita, 2ª visita e visita extra por bairro, sem somar uma visita com a outra. A linha TOTAL ao final de cada bairro soma apenas aquele bairro.</p><table><thead><tr><th>#</th><th>Bairro/Área</th><th>Visita</th>${rankingCabecalho}</tr></thead><tbody>${linhasRankingPorVisita}</tbody></table>`
