    ...
    <div className="dashboard-main">
      ...
      {erro ? <div className="error-state">{erro}. Verifique se a planilha está compartilhada para leitura por link.</div> : <>
        <section className="metrics">
          ...
          <article className="metric-card"><span className="metric-label">Ruas cadastradas</span><strong className="metric-value">{ruasCadastradas.toLocaleString("pt-BR")}</strong><div className="metric-detail">Nas abas dos bairros • cadastro da 1ª visita</div><div className="metric-detail">RESUMO POR RUA: {ruasNoResumo.toLocaleString("pt-BR")} • diferença {diferencaRuas >= 0 ? "+" : ""}{diferencaRuas.toLocaleString("pt-BR")}</div><div className="metric-detail">{ruasComVoto.toLocaleString("pt-BR")} com votos • {ruasSemVoto.toLocaleString("pt-BR")} sem votos para {candidato}</div></article>
