...
export default function Dashboard() {
  ...
  const camposPesquisasApuradas = camposPorCargo["Deputado Estadual"];
  const pesquisasApuradas = registrosFiltrados.reduce((total, registro) => total +
    camposPesquisasApuradas.reduce((subtotal, campo) => subtotal + Number(registro[campo] || 0), 0), 0);
  const casasVisitadas = registrosFiltrados.reduce((total, registro) => total + Number(registro["CASAS VISITADAS"] || 0), 0);
    ...
    <div className="dashboard-main">
      ...
      {erro ? <div className="error-state">{erro}. Verifique se a planilha está compartilhada para leitura por link.</div> : <>
        <section className="metrics">
          ...
          <article className="metric-card"><span className="metric-label">Pesquisas apuradas</span><strong className="metric-value">{pesquisasApuradas.toLocaleString("pt-BR")}</strong><div className="metric-detail">Soma estadual por bairro e visita</div></article>
          <article className="metric-card"><span className="metric-label">Casas visitadas</span><strong className="metric-value">{casasVisitadas.toLocaleString("pt-BR")}</strong><div className="metric-detail">Quantidade informada pela equipe</div></article>
        </section>
